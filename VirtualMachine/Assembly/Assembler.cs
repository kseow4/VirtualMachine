...
namespace VirtualMachine.Assembly
{
    public class Assembler
    {
        ...
        private static readonly OPCODE[] specialOps = FilterByCustomAttributeAndDescription<OPCODE>("Category", "Special Operations");
        ...
        public Assembler(VirtualMachine vm) => (VirtualMachine, Logger) = (vm, vm.Logger);
        ...
        public void FirstPass(List<List<object>> tokenizedLists)
        {
            ...
            VirtualMachine.DataSegmentSize = DirCounter + 1;
            //    VirtualMachine.ThreadQueue.ForEach(t => { if ((int)(THREAD)t.ID < VirtualMachine.MAX_THREAD) t.Deactivate(); });
            //  VirtualMachine.MAX_THREAD = (VirtualMachine.MEMORY_SIZE - VirtualMachine.SL) / VirtualMachine.STACK_SIZE;
            //VirtualMachine.ThreadQueue.ToList().ForEach(v => { if ((int)v.Key < VirtualMachine.MAX_THREAD) v.Value.Enable(); });
                ...
                try
                {
                    ...
                    switch (token)
                    {
                        ...
                        case string s when token is string:
                            if (tokenTypes.Count < 2) { Logger.LogException($"Label without instruction: \"{string.Join(" ", tokenTypes)}\" on line: [{listIndex}]"); }
                            if (VirtualMachine.SymbolTable.ContainsKey(token.ToString())) { Logger.LogException($"Duplicate Label: \"{token}\" on line: [{listIndex}]"); }
                        ...
                        default:
                            Logger.LogException($"{token} is {(token is null ? "null" : "invalid type")}: ");
                            break;
                    }
                }
                catch (Exception e) { Logger.LogException($"Error encountered on Line: [{listIndex}]: " + e.Message + $" [CodeCounter:{CodeCounter}, DirCounter:{DirCounter}]"); }
            }
            if (Logger.Count() > 0) { throw new Exception($"Error Count in First Pass: [{Logger.Count()}]"); }
        }
        ...
        public void SecondPass(List<List<object>> tokenizedLists)
        {
            ...
            foreach (List<object> tokens in tokenizedLists)
            {
                ...
                for (int i = 0; i < tokens.Count; i++)
                {
                    ...
                    try
                    {
                        ...
                        switch (token)
                        {
                            ...
                            case string s when token is string && VirtualMachine.SymbolTable.TryGetValue(s, out int address):
                                foreach (byte bite in BitConverter.GetBytes(address)) { InsertToken(TOKEN.Label, bite); }
                                break;
                    ...
                    catch (Exception e) { Logger.LogException(e.Message + "\t" + $"[{listIndex}] " + token.ToString() + ":(" + tokens[i] + ")\t" + "Error loading data into bytecode/memory."); }
                }
            }
            if (Logger.Count() > 0) { throw new Exception($"Error Count in Second Pass: [{Logger.Count(LOG.EXCEPTION)}]"); }
            VirtualMachine.CodeSegmentSize = DirCounter + 1; VirtualMachine.DataSegmentSize -= VirtualMachine.CodeSegmentSize;
        }
        ...
        private void CheckForErrors<T>(List<List<T>> lists)
        {
            ...
            {
                if (CheckTokens(lists[i])) { Logger.LogException($"Error occurred on line [{i}]:\t" + string.Join("\t", lists[i])); }
            }
        }
        ...
        private void PrintExceptions(Exception e)
        {
            ...
            foreach (string exception in Logger.Exceptions) { Console.WriteLine($"\t{++errorCount}. {exception}"); }
            Console.WriteLine();
        }
        ...
        private List<List<object>> Tokenizer(List<List<string>> lines)
        {
            ...
            {
                if (specialOps.Any(o => tokens.Contains(o.ToString()))) { tokens.Add("Null"); }
            //    if (tokens.Contains("TRP") || tokens.Contains("JMP") || tokens.Contains("JMR")) { tokens.Add("Null"); }
                ...
                {
                    Logger.LogException($"Error occurred in Tokenizer: [{string.Join(" ", tokens)}] => {e.Message}");
                }
            }
            return tokenLists;
        }
